...
export const sendVerficationEmail = (email) => async (dispatch) => {
	try {
		...
		const { data } = await axios.post(
			`${process.env.REACT_APP_BASE_URL}/api/users/confirm`,
			{ email },
			config
		);
	...
	async (dispatch, getState) => {
		try {
			...
			const { data } = await axios.get(
				`${process.env.REACT_APP_BASE_URL}/api/users/confirm/${emailToken}`
			);
	...
	(passwordToken, password) => async (dispatch) => {
		try {
			...
			const { data } = await axios.put(
				`${process.env.REACT_APP_BASE_URL}/api/users/reset`,
	...
	try {
		...
		if (userInfo.isSocialLogin) {
			...
			let { data } = await axios.post(
				`${process.env.REACT_APP_BASE_URL}/api/users/passport/data/`,
				{ id },
				config
			);
			dispatch({
...
export const listAllUsers =
	...
	async (dispatch, getState) => {
		try {
			...
			const { data } = await axios.get(
				`${process.env.REACT_APP_BASE_URL}/api/users?pageNumber=${pageNumber}`,
				config
			);
