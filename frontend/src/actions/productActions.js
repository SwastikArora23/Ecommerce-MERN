	...
	async (dispatch) => {
		try {
			...
			const { data } = await axios.get(
				`${process.env.REACT_APP_BASE_URL}/api/products?keyword=${keyword}&pageNumber=${pageNumber}&pageSize=${pageSize}`
			);
...
export const getTopRatedProducts = () => async (dispatch) => {
	try {
		...
		const { data } = await axios.get(`${process.env.REACT_APP_BASE_URL}/api/products/top`);
